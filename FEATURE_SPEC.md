...
### 修正 1：價格計算一致
- 菜單頁每項目須同時顯示 **日幣小計**（`item.price × item.qty`）與 **目標貨幣小計**（`item.price × item.qty × rate`）。
- 目前日幣只顯示單價（`item.price`），目標貨幣卻顯示小計，不一致。修正為**兩邊都顯示小計**。

### 修正 2：「返回」按鈕回到結果頁
- `#menu-back-scan` 原行為跳轉掃描器。修正為跳轉至 **`results-view`**，重新渲染 `appState.results`，讓使用者可回頭修改購物車數量與項目。
- 按鈕文字改為「**返回修改**」。

### 修正 3：原文與譯文字體大小一致
- 菜單項目卡片中，`nameTranslated` 與 `nameOriginal` 必須使用**相同字體大小**（`text-lg`），方便雙語對照閱讀。
- `nameTranslated`：`text-lg font-bold`
- `nameOriginal`：`text-lg font-medium`
